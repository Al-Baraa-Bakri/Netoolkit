...
import { setName , setType , addServer , toggleRouter , addFloor,clearBuildingState ,  distributedFloor,  deleteFloor , setRoomName , duplicateFloor , duplicateRoom , deleteRoom ,  minusDuplicateRoom , addRoom , setRoomNetworkPoints, updateFloorName } from "../../Store/slices/Netx/createBuildingSlice";
import { addBuilding } from "../../Store/slices/Netx/selectedProjectSlice";
...
function CreateBuildingModal({isOpen , toggleModal , onToast}: any) {
      const theme = useSelector((state: any) => state.theme.theme);
      const { name, floors , isCore , buildingName , isRouter , servers } = useSelector((state) => state.netxBuilding);
      ...
      const handleBuildingType = (type) => {
        if(buildings.filter((building) => building.isCore).length !== 0) {
      ...
      const handleAddBuilding = () => {
        ...
        toggleModal(); 
        dispatch(addBuilding({projectName , buildingName , isCore , floors , servers , isRouter})); 
        ...
        <div className="fixed z-10 inset-0 overflow-y-scroll ">
          ...
          />
               ...
               <div className="flex mt-7 w-full items-center justify-end gap-2">
                      ...
                      <button onClick={() => handleAddBuilding()}  className='flex py-5 px-4 w-26 h-[12px] rounded-md items-center justify-center bg-primary text-white text-base font-medium'>
                          Add A Building
                      </button>
               </div>
